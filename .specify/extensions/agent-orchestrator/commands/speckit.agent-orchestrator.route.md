...
2. If the index does not exist, run `/speckit.agent-orchestrator.index` first to build it
...
echo "  2     0.78   /speckit.agent-orchestrator.discover   extension"
...
echo "  • Run /speckit.agent-orchestrator.index to refresh the index"
...
- Re-run `/speckit.agent-orchestrator.index` after installing new extensions to update the index
...
```bash
> /speckit.agent-orchestrator.route "I want to create a specification for a login feature"
...
```bash
> /speckit.agent-orchestrator.route "set up git branching for my project"
...
```bash
> /speckit.agent-orchestrator.route "run the full development cycle end to end"
