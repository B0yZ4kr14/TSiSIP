<!--
SYNC IMPACT REPORT
Version change: 1.1.0 → 1.1.1 (patch bump)
Added sections: None
Modified principles:
  - Docker-image-first: added SHA256 digest pinning note
  - Module validity gate: expanded to explicitly forbid `sanity` and Kamailio-only modules
  - Security Expectations: added `no-new-privileges:true` requirement
  - Project Identity: added observability stack
...
- **Observability**: Prometheus + Grafana + Alertmanager + custom exporter

## Engineering Philosophy

1. **Docker-image-first**: All runtime components must be delivered as project-owned Docker images with SHA256-digest-pinned base images. Bare-metal or VM-first installations are rejected.
...
| Module validity | Only OpenSIPS 3.6 LTS documented modules. The `sanity` module and Kamailio-only modules are forbidden. | Block until corrected |
...
- Public entry points (5060/udp, 5060/tcp, 5061/tcp TLS) must authenticate all non-OPTIONS requests against PostgreSQL-backed subscriber credentials.
...
- Docker containers must run with `cap_drop: [ALL]`, minimal `cap_add`, and `security_opt: ["no-new-privileges:true"]`.

## Testing Expectations

- New OpenSIPS config changes must validate with `opensips -c`.
- New Docker images must pass healthchecks before deployment.
- Runtime SIP validation: OPTIONS 200 OK, INVITE 401 Unauthorized.
...
- Any documentation request producing or modifying canonical docs must follow the multi-agent validation swarm in `docs/TSiSIP-AGENT-ORCHESTRATION-PLAYBOOK.md`.
...
- **Published ports whitelist**: Only OpenSIPS (5060/udp, 5060/tcp, 5061/tcp) and RTPengine (10000-20000/udp) may publish host ports. All other services must remain on internal networks only.
...
**Version**: 1.1.1 | **Ratified**: 2026-05-17 | **Last Amended**: 2026-05-24
