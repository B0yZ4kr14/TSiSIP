...
description: Transform raw candidates into a locked L1/L2 capability model with security
  and QA context, plus a consolidated domain model and industry blueprint comparison.
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: brownkit:commands/discover.md
---
...
- **Defect profile**:
  ...
  - `change_velocity` — computed from git log (commits over last 6 months on
    ...
    `.specify/scripts/bash/git-churn.sh --root ./ --months 6`, which emits per-file
...
Use `.specify/templates/domain-model.md` as the skeleton. For each L1:
