...
<!-- Extension: onboard -->
<!-- Config: .specify/extensions/onboard/ -->
...
1. If `--dev <name>` is provided:
   ...
   - If not found, create a new profile at that path from `.specify/templates/profile.json`.
...
- **If the profile does not exist:** create it from `.specify/templates/profile.json` and fill in:
