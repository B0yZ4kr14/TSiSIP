...
docker ps --format '{{.Names}}' | grep -q 'tsisip-backup-1' && pass "Backup container" || fail "Backup container"

# PostgreSQL must NOT bind to 0.0.0.0
if ss -tlnp 2>/dev/null | grep ':5432' | grep -q '0.0.0.0:5432'; then
    fail "PostgreSQL exposed on 0.0.0.0"
else
    pass "PostgreSQL not exposed publicly"
fi

# Asterisk must NOT be public
if ss -tlnp 2>/dev/null | grep ':5060' | grep -q '0.0.0.0:5060'; then
    pass "Port 5060 exposed (OpenSIPS expected)"
else
    fail "Port 5060 not found"
fi
