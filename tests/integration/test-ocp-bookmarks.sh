...
DASH=$(curl -s ${HOST_HEADER:+-H "Host: $HOST_HEADER"} -c "$COOKIE_JAR" -b "$COOKIE_JAR" "$BASE/dashboard.php")
echo "$DASH" | grep -q "Bookmarks" && echo "[PASS] Bookmarks section visible"

# Toggle bookmark
BM=$(curl -s ${HOST_HEADER:+-H "Host: $HOST_HEADER"} -c "$COOKIE_JAR" -b "$COOKIE_JAR" \
...
# API docs page
API=$(curl -s ${HOST_HEADER:+-H "Host: $HOST_HEADER"} -c "$COOKIE_JAR" -b "$COOKIE_JAR" "$BASE/api-docs.php")
