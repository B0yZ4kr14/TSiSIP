...
description: Extract capability, security, and QA signals independently from code
  and external inputs; merge into confidence-rated evidence.
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: brownkit:commands/scan.md
---

<!--
Optional helpers for this phase (invoke as-needed, not all at once):
  .specify/scripts/bash/list-manifests.sh  --root <path>                       → SS2
  .specify/scripts/bash/find-secrets.sh    --root <path>                       → SS1
  .specify/scripts/bash/parse-coverage.sh  --report <path> [--format <fmt>]    → QS2
Each helper is also available under .specify/scripts/powershell/*.ps1.
-->
