...
SEARCH=$(curl -fsSL ${CURL_INSECURE_FLAG} ${CURL_HOST} -c "$COOKIE_JAR" -b "$COOKIE_JAR" "${BASE}/search.php")
...
# Search with query
SEARCH_Q=$(curl -fsSL ${CURL_INSECURE_FLAG} ${CURL_HOST} -c "$COOKIE_JAR" -b "$COOKIE_JAR" "${BASE}/search.php?q=admin")
echo "$SEARCH_Q" | grep -q "Subscribers" && echo "[PASS] Search with query works"

# Header profile link
HEADER=$(curl -fsSL ${CURL_INSECURE_FLAG} ${CURL_HOST} -c "$COOKIE_JAR" -b "$COOKIE_JAR" "${BASE}/dashboard.php")
