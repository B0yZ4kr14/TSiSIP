...
## Section 18: Backup Verification & Disaster Recovery (Feature 032)

### 18.1 Automated Backup Verification

Every backup generated by `scripts/backup-db.sh` now includes:
- **SHA-256 checksum** (`.sha256` file)
- **Metadata JSON** (`.meta.json` file) with timestamp, size, checksum, and verification status
- **Compression integrity check** via `gunzip -t`

To verify a backup manually:

```bash
# Verify the latest backup automatically
bash scripts/verify-backup.sh --backup backups/tsisip_db_$(date +%Y%m%d)_*.sql.gz

# Verify an encrypted backup
bash scripts/verify-backup.sh --backup backups/tsisip_db_YYYYMMDD_HHMMSS.sql.gz.enc --encrypted
```

The verification script:
1. Validates the checksum (if present)
2. Decrypts the backup (if encrypted)
3. Verifies compression integrity
4. Spins up a temporary PostgreSQL container
5. Restores the dump
6. Validates schema (all critical tables exist)
7. Reports row counts for key tables
8. Cleans up temporary resources

### 18.2 Backup Status Dashboard

Access the OCP backup status page at `/backup-status.php` (Administration menu).

The dashboard shows:
- Total backups and storage used
- Latest backup timestamp
- Verification status (PASS/PENDING/FAIL)
- Backup history table with age and size

### 18.3 Disaster Recovery Drills
...
This will:
1. Select the latest verified backup
2. Run `verify-backup.sh` against it
3. Measure total RTO (Recovery Time Objective)
4. Generate a markdown report in `reports/dr-drills/`

**Automated DR drills** are scheduled monthly via cron (configured via Ansible):
- Default: 1st of every month at 03:00
- Logs: `/var/log/tsisip-dr-drill.log`
- Reports: `reports/dr-drills/dr-drill-YYYYMMDD-HHMMSS.md`

### 18.4 Alerting

Prometheus alert rules for backup health:

| Alert | Condition | Severity |
|-------|-----------|----------|
| `TSiSIPBackupVerifyFailed` | `tsisip_backup_verify_success == 0` for > 5m | critical |
| `TSiSIPBackupTooOld` | Backup age > 25h | warning |
| `TSiSIPBackupSizeAnomaly` | Backup size < 1KB | warning |

### 18.5 Metrics

Backup metrics are exposed via `scripts/backup-metrics.sh`:

| Metric | Description |
|--------|-------------|
| `tsisip_backup_last_timestamp` | Unix timestamp of latest backup |
| `tsisip_backup_verify_success` | 1 if verified, 0 otherwise |
| `tsisip_backup_size_bytes` | Size of latest backup |
| `tsisip_backup_age_seconds` | Age of latest backup |
| `tsisip_backup_total_count` | Total backups in directory |

Generate metrics manually:
```bash
bash scripts/backup-metrics.sh
```

### 18.6 Interpreting Failures

| Symptom | Likely Cause | Action |
|---------|--------------|--------|
| Checksum mismatch | File corruption during write | Re-run backup; check disk health |
| Restore fails | Incompatible PostgreSQL version | Verify pg_dump version matches target |
| Table missing | Partial dump or schema drift | Run schema validation; check `pg_dump` flags |
| Verification timeout | Large backup or slow I/O | Increase timeout in `verify-backup.sh` |
| "No backups found" | Backup directory empty or misconfigured | Check `BACKUP_DIR` and cron schedule |

### 18.7 Production Restore Procedure

To restore from backup in production:

1. **Stop the application** (to prevent writes during restore):
   ```bash
   docker compose stop opensips ocp
   ```

2. **Restore the backup**:
   ```bash
   # For plain backups
   gunzip -c backups/tsisip_db_YYYYMMDD_HHMMSS.sql.gz | \
     docker compose exec -T postgres psql -U opensips -d opensips

   # For encrypted backups
   openssl enc -d -aes-256-cbc -pbkdf2 -in backups/...sql.gz.enc \
     -pass file:secrets/backup_encryption_key | gunzip | \
     docker compose exec -T postgres psql -U opensips -d opensips
   ```
...
3. **Verify restoration**:
   ...
   docker compose exec postgres psql -U opensips -d opensips -c "SELECT COUNT(*) FROM subscriber"
   ```

4. **Restart services**:
   ```bash
   docker compose up -d opensips ocp
   ```
